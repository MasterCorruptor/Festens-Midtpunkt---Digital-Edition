...
- `409` with `{ "error": "Deck id already exists" }` when an official or custom deck has the same exact ID.
- `409` with `{ "error": "Deck name already exists" }` when an official or custom deck name matches case-insensitively.

Validation response:

- `400` with `{ "error": "Deck data must be a JSON object" }` when the body is not a JSON object.
- `400` with `{ "error": "Deck id must use lowercase letters, numbers, or underscores" }` when `id` is missing, empty, not a string, or contains other characters.
...
- `400` with `{ "error": "Deck must contain at least one card" }` when `cards` is missing, is not an array, or is empty.
...
- `404` with `{ "error": "Deck not found" }` if the path ID is unknown.
- `400` when the replacement fails the same deck validation used for creation, except that the path supplies the existing ID.
- `409` with `{ "error": "Deck name already exists" }` when another official or custom deck already uses the submitted name, compared case-insensitively.
...
- `404` with `{ "error": "Deck not found" }` if the ID is unknown.
- `403` with `{ "error": "Official decks cannot be deleted" }` for an official deck.

## General error behavior and limitations

- Malformed JSON is handled by Express's JSON parser, outside the route-specific error objects.
- JSON parsing, directory access, file writes, and file deletion use synchronous calls without route-level error handling. Failures may become generic server errors or terminate processing, depending on the error.
...
- Unknown `/api/...` paths are not given a dedicated JSON 404 handler; static/fallback Express behavior applies.
